	...
	$(function(){
		...
		$("#dg").datagrid({
			...
			columns:[[
				...
				{field:"tname",title:"内容模板",width:100,align:"center"},
				{field:"uname",title:"发布人",width:100,align:"center"},
	...
	function openFormDialog(){
		...
		$.ajax({
			...
			success:function(result){
				...
				for(var i=0;i<result.length;i++){
					var row = result[i];
					$("#td").append("<option value="+row.id+">"+row.name+"</option>");
					if(array.length!= 0){
						if(row.id==array[0].tid){
							$("option").attr({selected:"selected"});
						}
					}
				}
	...
	<div id="dd" class="easyui-dialog" style="width: 400px;text-align: center;padding: 10px;" closed="true">
		<form id="ff" method="post">
			<table style="margin: 0 auto;">
				...
				<!-- <tr>
					<td class="right">内容模板：</td>
					<td><input type="text" name="tid" class="easyui-textbox" data-options="required:true"></td>
				</tr> -->
				
				<tr>
					<td class="right">内容模板：</td>
					<td>
					 	<select id="td" name="tid">
						</select>
					</td>
				</tr>
