...
public class PublicNumberFansController {
	...
	@RequestMapping("/add")
	public void add(PublicNumberFans t) {
		PrintWriter out = null;
		JSONObject jo = new JSONObject();
		try {
			out = response.getWriter();
			t.setId(GetIDTool.getUUID());
			publicNumberFansService.add(t);
			jo.put("state", 0);
			jo.put("msg", "新增信息成功");
		} catch (IOException e) {
			jo.put("state", -1);
	...
	public void update(PublicNumberFans t) {
		PrintWriter out = null;
		JSONObject jo = new JSONObject();
		try {
			out = response.getWriter();
			publicNumberFansService.update(t);
			jo.put("state", 0);
			jo.put("msg", "更新信息成功");
		} catch (IOException e) {
			jo.put("state", -1);
	...
	@ResponseBody
	public PublicNumberFans queryById(int id,String page,Map<String,Object> map) {
		PublicNumberFans t = publicNumberFansService.queryById(PublicNumberFans.class, id);
		map.put("page", page);
		map.put("Template", t);
		return t;
	}
	...
	public void queryByPage(String page) throws Exception {
		...
		List<PublicNumberFansVO> list = publicNumberFansService.queryViewByPage(sp, pageSize);
